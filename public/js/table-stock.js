        ...
        success: function(data){
            ...
            $('#print-stock').click(function() {
                ...
                var generateData = function(amount) {
                    var all = $(".printcode").map(function() {
                ...
                for (let x = 0; x < data['data'].length; x++) {
                    var name = {
                        Código: data['data'][x]['codProd'],
                        Nombre: data['data'][x]['nombreProducto'],
                        Descripcion: data['data'][x]['descripcion'],
                        Precio_Base: data['data'][x]['precioBase'],
                        Cantidad: data['data'][x]['cantidad'],
                        Precio_Venta: data['data'][x]['precioVenta']
                    };
                ...
                // direccion
                doc.setFontSize(10);
                doc.text(60, 30, 'Calle la mar 299 - Ica');
                // nombre empresa
                doc.setFontSize(15);
                doc.setFont('helvetica');
                doc.setFontType('bold');
                doc.text(60, 25, 'Importaciones NOARG');
                // FECHAS
                // desde
                doc.setFontType('');
                doc.setFontSize(12);
                ...
                doc.addImage(imgData, "JPEG", 15, 10, 45, 45);
                // Genera la tabla
                doc.table(15, 60, generateData(5), headers, { autoSize: false, fontSize: 6 });
