        ...
        success: function(data){
            ...
            $('#print-stock').click(function() {
                ...
                doc.autoTable({
                    margin: { top: 60 },
                    theme: 'grid',
                    headStyles: {lineColor: [0, 0, 0], textColor: [0, 0, 0], fillColor: [255, 255, 255], lineWidth: 0.1, fontSize: 7},
...
// al presionar el boton imprimier del reporte de catalogo
$.ajax({
    type:"GET",
    datatype: "json",
    url: `http://appdemo1.solarc.pe/api/Productos/DownloadFilePDF`,
    success: function(data_catalog){
        $("#print-catalog").attr("download", "reporte_catalogo_" + getYr + "-" + getMonFormat + "-" + getDy);
        // al presionar el boton imprimir en el modal reporte de stock
        $('#print-catalog').click(function() {
            $("#print-catalog").attr("href", 'data:application/pdf;base64,' + data_catalog.data.oFile);
        })
    }

});

$('#upload-xls').click(function (e) { 
    e.preventDefault();
    $('#inputExcel').val('');
});
// subir excel a base de datos
$('#addExcel').click(function () {
    const selectedExcel = document.getElementById('inputExcel'); // input del archivo
    const userId = localStorage.getItem('userId'); // obtener usuario del localstorage
    
    // Formulario agregar excel
    var formAddExel = new FormData();
    formAddExel.append("FileData", selectedExcel.files[0]);
    formAddExel.append("IdUsuarioCreacion", `${userId}`);

    var settingsAddExcel = {
        "url": "http://appdemo1.solarc.pe/api/Productos/UploadFileXLS",
        "method": "POST",
        "timeout": 0,
        "processData": false,
    ...
    // peticion ajax
    $.ajax(settingsAddExcel)
    // si se completa el envio
    .done(function (response) {
        ...
        $("#alert").append(`
            ...
            <div class="alert alert-success alert-dismissible d-flex align-items-center" role="alert" mt-5>
                <svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Success:"><use xlink:href="#check-circle-fill"/></svg>
                <div>
                    <strong>Se agrego!</strong> correctamente el archivo.
    ...
    .fail(function() {
        ...
        $("#alert").append(`
            ...
            <div class="alert alert-danger d-flex alert-dismissible align-items-center" role="alert">
                ...
                <div>
                    <strong>Error!</strong> no se pudo agregar el archivo.
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>
            </div>
        `);
