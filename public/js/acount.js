$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    node4.className = `material-icons del-users deur${i}`;
                    node4.setAttribute("data-bs-toggle", "modal");
                    node4.setAttribute("data-bs-target", "#tableModal");

                    document.getElementById("tb-body").appendChild(node);

                    $(`.deur${i}`).click(function (e) { 
                        e.preventDefault();

                        $(".user-del-table").html(`<strong>${data[i].userName}</strong>`);
                        $("#dId").val(data[i].idUsuario);
                        $("#dUs").val(data[i].userName);
                    });
                }
            },
    ...
    $("#acceptDeleteTable").click(function (e) { 
        e.preventDefault();
        const deleteIdTable = $("#dId").val();
        const deleteUserTable = $("#dUs").val();
        // console.log(deleteIdTable, deleteUserTable);
        ...
        fetch('http://appdemo1.solarc.pe/api/Account/UsuariosActualizar', {
            ...
            body: JSON.stringify({
                "userName": deleteUserTable,
                "userPass": 0,
                "nombres": "string",
                "idPerfil": deleteIdTable,
                "idTienda": 1,
                "estadoReg": 0
            })
        })
