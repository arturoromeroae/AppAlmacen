        ...
        success: function (response) {
            ...
            for (let i = 0; i < res.length; i++) {
                if (getDataTable.idVentaCab == res[i].idVentaCab) {
                    counter++
                    document.getElementById(`impr`).innerHTML += `<input value="${res[i].nombreProducto}" hidden>`
                    
                    if (counter < 2) {
                        $(`#dateCotize`).attr("value", res[i].fecha);
                        $(`#subtCotize`).attr("value", res[i].subTotal);
                    }
                }
            }

            $(`#nCotize`).text(`${getDataTable.numero}`);

            $(`#cancl-cotize`).click(function () { 
                document.getElementById(`impr`).innerHTML = ''
                $(`#loadDelete`).hide();
                $(`#deleteInfo`).show();
            });

            $(`#x-cotize`).click(function () { 
                document.getElementById(`impr`).innerHTML = ''
                $(`#loadDelete`).hide();
                $(`#deleteInfo`).show();
            });

            $(`#del-cotize`).click(function () {
                
                for (let i = 0; i < res.length; i++) {
                    const ids = {
                        "idVentaCab": getDataTable.idVentaCab,
                        "idProducto": res[i].idProducto,
                        "cantidad": res[i].cantidad,
                        "precioVenta": res[i].precioVenta,
                        "valorVenta": res[i].subTotal,
                ...
                fetch('http://appdemo1.solarc.pe/api/Cotiza/InsertaCotiza', {
                    ...
                    body: JSON.stringify({
                        "fecha": "2021-12-30T04:54:04.966Z",
                        "idCliente": 0,
                        "tipoVenta": 4,
                        "subTotal": 0,
                        "igv": 0.18,
                        "total": getDataTable.total,
                        "vuelto": 0,
                        "porcDscto": 0,
                        "valorDscto": 0,
                        "valorVenta": 0,
                        ...
                        "rucCliente": "string",
                        "razonSocial": getDataTable.razonSocial,
                        "idOrigen": 1,
                        "isAnulado": 1,
                        "idVentaCab": getDataTable.idVentaCab,
                        "cotizaDet": productsDetail
                    })
                })
                .then(res => res.json())
                .then(data => {
